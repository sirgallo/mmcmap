...
/*
CalculateHashForCurrentLevel calculates the hash for value based on what level of the trie the operation is at. Hash is reseeded every 6 levels
...
/*
getSparseIndex gets the index at a particular level in the trie. Pass through function.
...
/*
getPosition calculates the position in the child node array based on the sparse index and the current bitmap of internal node. The sparse index is calculated using the hash and bitchunk size. 
A mask is calculated by performing a bitwise left shift operation, which shifts the binary representation of the value 1 the number of positions associated with the sparse index value and 
then subtracts 1, which creates a binary number with all 1s to the right sparse index positions. the mask is then applied the bitmap and the resulting isolated bits are the 1s right of the 
sparse index. The hamming weight, or total bits right of the sparse index, is then calculated.
...
/*
GetIndexForLevel determines the local level for a hash at a particular seed
...
/*
GetIndex gets the index at a particular level in the trie by shifting the hash over the chunk size t (5 for 32 bits), and then applying a mask to the shifted hash to return an index 
mapped in the sparse index. Non-zero values in the sparse index represent indexes where nodes are populated. The mask is the value 31 in binary form.
...
/*
setBit performs a logical xor operation on the current bitmap and the a 32 bit value where the value is all 0s except for at the position of the incoming index. Essentially flips the bit if incoming is 1 and
bitmap is 0 at that position, or 0 to 1. if 0 and 0 or 1 and 1, bitmap is not changed.
...
/*
IsBitSet determines whether or not a bit is set in a bitmap by taking the bitmap and applying a mask with a 1 at the position in the bitmap to check. A logical and operation is applied and
if the value is not equal to 0, then the bit is set.
...
func (pcMap *PCMap) PrintChildren() {
	pcMap.printChildrenRecursive(&pcMap.Root, 0)
}

func (pcMap *PCMap) printChildrenRecursive(node *unsafe.Pointer, level int) {
	...
	for idx, child := range currNode.Children {
		if child != nil {
			...
			pcMap.printChildrenRecursive(&childPtr, level + 1)
		}
	}
}