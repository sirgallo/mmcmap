...
func extendTable(orig []*MMCMapINode, bitMap [8]uint32, pos int, newNode *MMCMapINode) []*MMCMapINode {
	...
	newTable := make([]*MMCMapINode, tableSize)
...
func (mmcMap *MMCMap) getPosition(bitMap [8]uint32, index byte, level int) int {
	subBitmapIndex := index / 32
	indexInSubBitmap := index % 32
...
func shrinkTable(orig []*MMCMapINode, bitMap [8]uint32, pos int) []*MMCMapINode {
	var tableSize int 
	for _, subBitmap := range bitMap {
		tableSize += calculateHammingWeight(subBitmap)
	}
...
func (mmcMap *MMCMap) printChildrenRecursive(node *MMCMapINode, level int) error {
	if node == nil { return nil }

	for idx := range node.Children {
		childPtr := node.Children[idx]
		child, desErr := mmcMap.ReadINodeFromMemMap(childPtr.StartOffset)
